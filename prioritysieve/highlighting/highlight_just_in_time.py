...
from .. import prioritysieve_config, prioritysieve_globals, text_preprocessing
...
def highlight_morphs_jit(
    ...
    if (
        ...
        not in [
            "ps-highlight",
            "ps-highlight-furigana",
            "ps-highlight-kanji",
            "ps-highlight-kana",
        ]
        or field_name == prioritysieve_globals.EXTRA_FIELD_HIGHLIGHTED
    ):
        return field_text

    am_config_filter: PrioritySieveConfigFilter | None = (
        prioritysieve_config.get_matching_filter(context.note())
    )
...
def _dehtml(
    text: str,
    am_config: PrioritySieveConfig | None = None,
...
def _get_ruby_type(filter_name: str) -> type[Ruby]:
    ...
    if filter_name == "ps-highlight-furigana":
        return FuriganaRuby
    if filter_name == "ps-highlight-kanji":
        return KanjiRuby
    if filter_name == "ps-highlight-kana":
        return KanaRuby
    return TextRuby
